...
Before running a command, read [references/runtime.md](references/runtime.md). It contains the exact `RRC_COMMAND` for this machine. Replace `<RRC_COMMAND>` below with that command. If the runtime is not configured, ask the user to run `rrc setup` rather than improvising an installation.
...
1. Fingerprint the exact PDF:
   ...
   ```bash
   <RRC_COMMAND> manifest PAPER.pdf
   ```
...
3. Read content only through the receipt-producing reader:
   ...
   ```bash
   <RRC_COMMAND> read PAPER.pdf --pages 1-3,7 --ledger .rrc/access.jsonl
   ```

4. Build a report matching [references/reading-report.schema.json](references/reading-report.schema.json). For each substantive claim:
...
5. Verify outside the reasoning step:
   ...
   ```bash
   <RRC_COMMAND> verify report.json --ledger .rrc/access.jsonl --pdf PAPER.pdf
   ```
